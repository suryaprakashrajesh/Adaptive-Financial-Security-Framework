...
# Global variables
users: dict[str, Any] = {}
atm_balance: float = 50000.00
atm_history: list[dict[str, Any]] = []
    ...
    if os.path.exists(DATA_FILE):
        try:
            with open(DATA_FILE, 'r') as f:
                ...
                for txs in user_history.values():
                    for t in txs:
                        if t.get('type') == 'ATM':
                            t['type'] = 'Withdrawal'
...
def history():
    ...
    if user_role == 'admin':
        ...
        user_transactions = get_enriched_user_transactions(username)
        return render_template('history.html', 
...
def dashboard():
    ...
    if user_role == 'admin':
        ...
        recent_transactions = atm_history[-5:] if atm_history else []  # type: ignore
        ...
        user_transactions = get_enriched_user_transactions(username)[-5:] if username in user_history else []  # type: ignore
        return render_template('dashboard.html',
                             username=username,
                             is_admin=False,
...
def export_data(format):
    ...
    if format == 'json':
        ...
        for username, data in users.items():
            writer.writerow([username, data.get('role', 'user'), data.get('balance', 0.0)])
        ...
        # User Data Sheet
        ws1 = wb.active
        ws1.title = "Users Data"
        ws1.append(["Username", "Role", "Balance"])
        for username, data in users.items():
            ws1.append([username, data.get('role', 'user'), data.get('balance', 0.0)])
            
        # ATM Balance Sheet
        ws2 = wb.create_sheet(title="ATM Balance")
        ws2.append(["ATM Balance"])
        ws2.append([atm_balance])
        
        # ATM History Sheet
        ws3 = wb.create_sheet(title="ATM History")
        ws3.append(['ID', 'User', 'Amount', 'Date', 'Status', 'Type'])
        for t in atm_history:
            ws3.append([t['id'], t['name'], t['amount'], t['date'], t['status'], t['type']])
            
        # User History Sheet
        ws4 = wb.create_sheet(title="User History")
        ws4.append(['Username', 'ID', 'Amount', 'Date', 'Status', 'Type'])
        for username, transactions in user_history.items():
            for t in transactions:
                ws4.append([username, t['id'], t['amount'], t['date'], t['status'], t['type']])
        ...
        return send_file(
            ...
            as_attachment=True,
            download_name=f'qratm_data_{datetime.now().strftime("%Y%m%d_%H%M%S")}.xlsx'
        )
        ...
        try:
            ...
            pdf.cell(0, 10, "QR ATM - Transaction History and Data", new_x="LMARGIN", new_y="NEXT", align='C')
            pdf.ln(5)
            
            # ATM Summary
            pdf.set_font("helvetica", "B", 14)
            pdf.cell(0, 10, "ATM Summary", new_x="LMARGIN", new_y="NEXT")
            pdf.set_font("helvetica", "", 12)
            pdf.cell(0, 8, f"Total ATM Balance: {atm_balance}", new_x="LMARGIN", new_y="NEXT")
            pdf.ln(5)
            
            # ATM History
            pdf.set_font("helvetica", "B", 14)
            pdf.cell(0, 10, "ATM Transactions", new_x="LMARGIN", new_y="NEXT")
            pdf.set_font("helvetica", "B", 10)
            pdf.cell(10, 8, "ID", border=1)
            pdf.cell(30, 8, "User", border=1)
            pdf.cell(25, 8, "Amount", border=1)
            pdf.cell(45, 8, "Date", border=1)
            pdf.cell(30, 8, "Status", border=1)
            pdf.cell(25, 8, "Type", border=1, new_x="LMARGIN", new_y="NEXT")
            ...
            for t in atm_history:
                ...
                pdf.cell(30, 8, str(t.get('name', 'N/A')), border=1)
                pdf.cell(25, 8, str(t['amount']), border=1)
                pdf.cell(45, 8, str(t['date']), border=1)
                pdf.cell(30, 8, str(t['status']), border=1)
                pdf.cell(25, 8, str(t['type']), border=1, new_x="LMARGIN", new_y="NEXT")
            ...
            return send_file(
                ...
                as_attachment=True,
                download_name=f'qratm_data_{datetime.now().strftime("%Y%m%d_%H%M%S")}.pdf'
            )
        except Exception as e:
            app.logger.error(f"Error generating PDF: {str(e)}")
            return f"Error generating PDF: {str(e)}", 500

def get_enriched_user_transactions(username):
    my_transactions = user_history.get(username, [])
    if not my_transactions:
        return []
        
    current_balance = users.get(username, {}).get('balance', 0.0)
    ...
    running_balance = current_balance - total_change
...
@app.route('/export_my_history/<format>')
@user_required
def export_my_history(format):
    if format not in ['pdf', 'xlsx']:
        return "Invalid format", 400
        
    username = session.get('username')
    
    # Retrieve all user transactions and enrich with running balance
    my_transactions = get_enriched_user_transactions(username)
    
    if format == 'xlsx':
        wb = Workbook()
        ws = wb.active
        ws.title = "My Transaction History"
        ws.append(['ID', 'Amount', 'Date', 'Status', 'Type', 'Balance Before', 'New Balance'])
        for t in my_transactions:
            ws.append([t['id'], t['amount'], t['date'], t['status'], t['type'], t['balance_before'], t['balance_after']])
        ...
        return send_file(
            ...
            mimetype='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
            as_attachment=True,
            download_name=f'my_transactions_{datetime.now().strftime("%Y%m%d_%H%M%S")}.xlsx'
        )
    elif format == 'pdf':
        try:
            pdf = FPDF()
            ...
            pdf.set_font("helvetica", "B", 16)
            pdf.cell(0, 10, f"Transaction History - {username}", new_x="LMARGIN", new_y="NEXT", align='C')
            pdf.ln(5)
            
            pdf.set_font("helvetica", "B", 10)
            pdf.cell(10, 8, "ID", border=1)
            pdf.cell(20, 8, "Amount", border=1)
            pdf.cell(45, 8, "Date", border=1)
            pdf.cell(25, 8, "Status", border=1)
            pdf.cell(25, 8, "Type", border=1)
            pdf.cell(32, 8, "Balance Before", border=1)
            pdf.cell(33, 8, "New Balance", border=1, new_x="LMARGIN", new_y="NEXT")
            ...
            for t in my_transactions:
                ...
                pdf.cell(10, 8, str(t['id']), border=1)
                pdf.cell(20, 8, str(t['amount']), border=1)
                pdf.cell(45, 8, str(t['date']), border=1)
                pdf.cell(25, 8, str(t['status']), border=1)
                pdf.cell(25, 8, str(t['type']), border=1)
                pdf.cell(32, 8, f"{t['balance_before']:.2f}", border=1)
                pdf.cell(33, 8, f"{t['balance_after']:.2f}", border=1, new_x="LMARGIN", new_y="NEXT")
            ...
            return send_file(
                ...
                mimetype='application/pdf',
                as_attachment=True,
                download_name=f'my_transactions_{datetime.now().strftime("%Y%m%d_%H%M%S")}.pdf'
            )
        except Exception as e:
            app.logger.error(f"Error generating PDF: {str(e)}")
            return f"Error generating PDF: {str(e)}", 500
